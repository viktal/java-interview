...
[Переопределение, перегрузка](https://java-course.ru/begin/override-overload/)
Переопределение (@Override - анотация) - переписать метод родителя на свой лад
Перегрузка - несколько методов с разными типами переменных
...
[Вложенные классы](https://javarush.ru/groups/posts/vidy-vlozhennyh-klassov)
...
Класс `StringBuilder` был добавлен в Java 5 и он во всем идентичен классу `StringBuffer` за исключением того, что он не синхронизирован (не потокобезопасен) и поэтому его методы выполняются значительно быстрей.
